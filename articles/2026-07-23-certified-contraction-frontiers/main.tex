...
result:} lowering $\scT$ below the natural space cost of good trees
degrades $\tc$ steeply, while raising it---all the way to
$\scT=\infty$, i.e., deleting the memory objective---costs nothing. The
harm is one-directional. At the shipped default the damage is $4.3$
$\log_2$ units on reg3\_250 and $9.7$ on sycamore\_m20: a $20\times$ and
$800\times$ flop overhead, recovered by a one-line change. An ablation
within one of our search mechanisms confirms the attribution: holding
everything else fixed and moving the penalty target to the natural space
cost moved reg3\_250 from $\tc=56.8$ to $40.0$, while that mechanism's
novel component contributed $0.002$.

The practical rule is: \emph{never anneal toward a memory target below
the natural space cost of good trees; in the absence of a real memory
budget, use no target at all.} The corresponding fix (default
$\scT=\infty$, with finite values only when a physical memory budget is
supplied) applies to omeco and, we expect, upstream implementations
sharing the default.
...
\begin{figure}[t]
  ...
  space cost of optimum-quality trees. Left of that line, quality
  degrades steeply; at and beyond it, including $\scT=\infty$, the
  frontier is reached. The shipped default is $\scT=20$.}
  \label{fig:sweep}
\end{figure}
