...
WALL_CAP_S=600
BUILD_CAP_S=60
RUN_BUDGET_MS=${ATT_DEV_BUDGET_MS:-47000}
...
if [[ ${ATT_BENCH_GUARDED:-0} != 1 ]]; then
  ...
  echo "work match: ATT_MAX_SWEEPS=${SWEEP_CAP}; serial 1-core runs; arms per instance: 061,059,switch-{15,25,40}%" >&2
fi
...
if [[ ! -f "$INSTANCES_DIR/ksg.json" || ! -f "$INSTANCES_DIR/surfacecode_d13.json" ]]; then
  echo "instances_dir must contain ksg.json and surfacecode_d13.json" >&2
  exit 2
fi
...
TMP_DIR=$(mktemp -d "${TMPDIR:-/tmp}/omeco-attempt062.XXXXXX")
...
for instance in ksg surfacecode_d13; do
  graph="$INSTANCES_DIR/$instance.json"
  for arm in parent061 front059 switch15 switch25 switch40; do
    log="$TMP_DIR/${instance}-${arm}.stderr"
    tree="$TMP_DIR/${instance}-${arm}.json"
    if [[ $arm == parent061 ]]; then
      fraction=parent
      toggles=(ATT_PARENT=1)
    elif [[ $arm == front059 ]]; then
      fraction=front
      toggles=(ATT_FRONT_ONLY=1)
    else
      percent=${arm#switch}
      fraction=$(python3 -c 'import sys; print(int(sys.argv[1]) / 100)' "$percent")
      toggles=("ATT_SWITCH_FRAC=$fraction")
    fi
    ...
    python3 "$SUMMARIZER" row "$instance" "$arm" "$fraction" "$SWEEP_CAP" "$log" "$tree" >> "$OUT"
  done
done
