...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // [030 debug] The validator scorer's Python recursion limit that rejected
    // trees taller than ~475 is FIXED in the main repo (scorer.py now handles
    // height ~10^5). Parent 029's defensive height-475 output cap threw away
    // exactly the deep, low-tc MPS-sweep trees the warm path produces on RQC
    // instances. Raise the cap to 100_000 so deep trees are admissible; the
    // `nested_height(&cand) <= HEIGHT_CAP` write guards are kept intact but now
    // effectively never fire.
    const HEIGHT_CAP: usize = 100_000;
    ...
    // [030 debug] Detect RQC-structured instances (rank-1 boundary vectors +
    // rank-4 gate tensors, the layered-circuit shape `structured_seed` keys on).
    // For these, the structured-seed + WARM-refinement path is now the DEFAULT:
    // it reaches deep low-tc MPS-sweep trees quickly, and those trees are now
    // admissible (height cap raised). reg3-like / generic instances still take
    // the library-TreeSA doubling default below (warm loses on the expander).
    let is_rqc_structured = {
        let boundary = (0..n).filter(|&t| dedup(&ixs[t]).len() == 1).count();
        let gates = (0..n).filter(|&t| dedup(&ixs[t]).len() == 4).count();
        boundary >= 4 && gates >= 4
    };
    eprintln!("[030] is_rqc_structured={is_rqc_structured} experiment={experiment}");

    if !experiment && !is_rqc_structured {
